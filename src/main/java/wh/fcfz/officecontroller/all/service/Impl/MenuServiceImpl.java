...
public class MenuServiceImpl extends ServiceImpl<MenuMapper, Menu> implements MenuService {
    ...
    public Result<List<MenuVo>> getMenuTree() {
        ...
        Map<Integer, List<Menu>> parentMenuMap = dbmenuList.stream()
                .collect(Collectors.groupingBy(Menu::getParentId)); // 按 parentId 分组

        // 获取所有父菜单（parent_id 为 0 的菜单）
        List<MenuVo> topLevelMenus = dbmenuList.stream()
                .filter(menu -> menu.getParentId() == 0)
                .map(menu -> convertToMenuVo(menu, parentMenuMap)) // 修改为使用 MenuVo
                .collect(Collectors.toList());

        return new Result<>(ResponseEnum.SUCCESS, topLevelMenus);
    }

    // 将 Menu 转换为 MenuVo 并递归构建树形结构
    private MenuVo convertToMenuVo(Menu menu, Map<Integer, List<Menu>> parentMenuMap) {
        // 获取当前菜单的子菜单列表
        List<MenuVo> sonMenus = Optional.ofNullable(parentMenuMap.get(menu.getMenuId()))
                .orElse(Collections.emptyList())  // 如果没有子菜单则返回空列表
                .stream()
                .map(subMenu -> convertToMenuVo(subMenu, parentMenuMap))  // 递归调用转换子菜单
                .collect(Collectors.toList());

        // 构建 MenuVo 并将子菜单列表赋值给 sonMenus
        return new MenuVo(menu.getPath(), menu.getMenuName(), menu.getMenuLabel(), menu.getIcon(), menu.getComponent(), sonMenus);
    }
}
